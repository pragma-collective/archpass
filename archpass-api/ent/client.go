...
import (
	...
	"github.com/pragma-collective/archpass/ent/migrate"
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Attendee, c.Event, c.Order, c.Ticket, c.Transaction, c.User,
	} {
		n.Use(hooks...)
	}
}
...
// QueryOrders queries the orders edge of a Event.
func (c *EventClient) QueryOrders(e *Event) *OrderQuery {
...
// QueryEvent queries the event edge of a Order.
func (c *OrderClient) QueryEvent(o *Order) *EventQuery {
	query := (&EventClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := o.ID
		step := sqlgraph.NewStep(
...
// QueryTicket queries the ticket edge of a Order.
func (c *OrderClient) QueryTicket(o *Order) *TicketQuery {
	query := (&TicketClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := o.ID
		step := sqlgraph.NewStep(
...
// QueryOrders queries the orders edge of a Ticket.
func (c *TicketClient) QueryOrders(t *Ticket) *OrderQuery {
	query := (&OrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := t.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Attendee, Event, Order, Ticket, Transaction, User []ent.Hook
	}
	inters struct {
		Attendee, Event, Order, Ticket, Transaction, User []ent.Interceptor
	}
)

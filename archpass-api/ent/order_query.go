...
func (oq *OrderQuery) QueryEvent() *EventQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, order.EventTable, order.EventColumn),
		)
...
func (oq *OrderQuery) QueryTicket() *TicketQuery {
	query := (&TicketClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, order.TicketTable, order.TicketColumn),
		)
		fromU = sqlgraph.SetNeighbors(oq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withEvent:  oq.withEvent.Clone(),
		withTicket: oq.withTicket.Clone(),
...
func (oq *OrderQuery) WithEvent(opts ...func(*EventQuery)) *OrderQuery {
	query := (&EventClient{config: oq.config}).Query()
...
func (oq *OrderQuery) WithTicket(opts ...func(*TicketQuery)) *OrderQuery {
	query := (&TicketClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withEvent; query != nil {
		if err := oq.loadEvent(ctx, query, nodes, nil,
			func(n *Order, e *Event) { n.Edges.Event = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withTicket; query != nil {
		if err := oq.loadTicket(ctx, query, nodes, nil,
			func(n *Order, e *Ticket) { n.Edges.Ticket = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*Order, init func(*Order), assign func(*Order, *Event)) error {
...
func (oq *OrderQuery) loadTicket(ctx context.Context, query *TicketQuery, nodes []*Order, init func(*Order), assign func(*Order, *Ticket)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		fk := nodes[i].TicketID
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withEvent != nil {
			_spec.Node.AddColumnOnce(order.FieldEventID)
		}
		if oq.withTicket != nil {
			_spec.Node.AddColumnOnce(order.FieldTicketID)
		}
	}

...
type UserQuery struct {
	...
	withAttendeeTickets *AttendeeQuery
...
// QueryAttendeeTickets chains the current query on the "attendee_tickets" edge.
func (uq *UserQuery) QueryAttendeeTickets() *AttendeeQuery {
	query := (&AttendeeClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAttendeeTickets: uq.withAttendeeTickets.Clone(),
...
// WithAttendeeTickets tells the query-builder to eager-load the nodes that are connected to
// the "attendee_tickets" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithAttendeeTickets(opts ...func(*AttendeeQuery)) *UserQuery {
	query := (&AttendeeClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withAttendeeTickets = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAttendeeTickets; query != nil {
		if err := uq.loadAttendeeTickets(ctx, query, nodes,
			func(n *User) { n.Edges.AttendeeTickets = []*Attendee{} },
			func(n *User, e *Attendee) { n.Edges.AttendeeTickets = append(n.Edges.AttendeeTickets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadAttendeeTickets(ctx context.Context, query *AttendeeQuery, nodes []*User, init func(*User), assign func(*User, *Attendee)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attendee.FieldUserID)
	}
	query.Where(predicate.Attendee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AttendeeTicketsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
